...
cm = table(predict=ypred$class, truth=test$labs_svm)
mean(ypred$class== test$labs_svm )

#validation
ypred=predict(lda.fit ,valid[,vars_keep])
cm = table(predict=ypred$class, truth=valid$labs_svm)
...
brf.fit<-randomForest(as.factor(test$labs_svm) ~.,
              data=test[,vars_keep],
              sampsize=make.size(as.factor(test$labs_svm)))

#predicting
brf.pred=predict(brf.fit, test[,vars_keep])
brf.class = brf.pred

#test
table(brf.class, test[,vars_keep]$labs_svm)
#overall classification rate for training
mean(brf.class==as.factor(as.numeric(test$labs_svm)))


ypred=predict(brf.fit, test[,vars_keep])
cm = table(predict=ypred, truth=test$labs_svm)
mean(ypred== test$labs_svm )

#validation
ypred=predict(brf.fit ,valid[,vars_keep])
cm = table(predict=ypred, truth=valid$labs_svm)
mean(ypred== valid$labs_svm )
...
ypred=predict(brf.fit, mat[,vars_keep])
cm = table(predict=ypred, truth=mat$labs_svm)
