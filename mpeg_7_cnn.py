...
import numpy as np#typical package (Kinser 8018)
import itertools as it #I don't think that this is used
import scipy.misc as sm #typical image package (Kinser 8018)
import convert_cnn as cvt #custom functions
...
dirs = ["/home/billy/Documents/Research/data/mpeg7/bird", "none"]
bird = cvt.GetAllImagesCNN(dirs)

dirs = ["/home/billy/Documents/Research/data/mpeg7/bone", "none"]
bone = cvt.GetAllImagesCNN(dirs)

dirs = ["/home/billy/Documents/Research/data/mpeg7/brick", "none"]
brick = cvt.GetAllImagesCNN(dirs)

dirs = ["/home/billy/Documents/Research/data/mpeg7/cam", "none"]
cam = cvt.GetAllImagesCNN(dirs)

dirs = ["/home/billy/Documents/Research/data/mpeg7/cup", "none"]
...
##################################
## training sample size = 56
##################################

#sample size
n=56
#initialize arrays to hold results
train_vals=np.zeros(5)
test_vals=np.zeros(5)
...
n=56
#training datasets
train1<-matrix(nrow=5, ncol=n)
train2<-matrix(nrow=5, ncol=n)
train3<-matrix(nrow=5, ncol=n)
train4<-matrix(nrow=5, ncol=n)
train5<-matrix(nrow=5, ncol=n)

#testing training sets
test1<-matrix(nrow=5, ncol=80-n)
test2<-matrix(nrow=5, ncol=80-n)
test3<-matrix(nrow=5, ncol=80-n)
test4<-matrix(nrow=5, ncol=80-n)
test5<-matrix(nrow=5, ncol=80-n)

for(i in 1:5){
    temp<-0:79

    train1[i,]<-sample(0:79,  n)
    test1[i,]<-temp[-(train1[i,]+1)]

    train2[i,]<-sample(0:79, n)
    test2[i,]<-temp[-(train2[i,]+1)]

    train3[i,]<-sample(0:79, n)
    test3[i,]<-temp[-(train3[i,]+1)]

    train4[i,]<-sample(0:79, n)
    test4[i,]<-temp[-(train4[i,]+1)]

    train5[i,]<-sample(0:79, n)
...
for j in range(0,5):
    ...
    #add model layers
    #first C layer
    #Rectified Linear Unit activation, 64 nodes, 3x3 filter matrix, 180x180x1 images
    ...
    conf_mat = confusion_matrix(vals_truth, vals_pred)
    print("Training Confusion Matrix for random split = " +str(j))
    print(conf_mat)
    ...
    conf_mat = confusion_matrix(vals_truth, vals_pred)
    print("Testing Confusion Matrix for random split = " +str(j))
    print(conf_mat)
