#functions for training svm-tree using data.tree package

#setup on R version 3.4.4

#install packages for ARGO
#install.packages('tidyverse',repos="http://cran.r-project.org", lib.loc='/scratch/wlamber2/Rpckgs')
#install.packages('sys',repos="http://cran.r-project.org", lib.loc='/scratch/wlamber2/Rpckgs')
#install.packages('rlang',repos="http://cran.r-project.org", lib='/scratch/wlamber2/Rpckgs')
#install.packages('data.tree',repos="http://cran.r-project.org", lib='/scratch/wlamber2/Rpckgs')
#install.packages('e1071',repos="http://cran.r-project.org", lib='/scratch/wlamber2/Rpckgs')
#install.packages('glmnet',repos="http://cran.r-project.org", lib='/scratch/wlamber2/Rpckgs')
#install.packages('randomForest',repos="http://cran.r-project.org", lib='/scratch/wlamber2/Rpckgs')
#install.packages('caret',repos="http://cran.r-project.org", lib='/scratch/wlamber2/Rpckgs')

#packages
#library(data.tree, lib='/scratch/wlamber2/Rpckgs')
#library(e1071, lib='/scratch/wlamber2/Rpckgs')
#library(glmnet, lib='/scratch/wlamber2/Rpckgs')
#library(randomForest, lib='/scratch/wlamber2/Rpckgs')
#library(caret, lib='/scratch/wlamber2/Rpckgs')
library(data.tree)
library(e1071)
library(rsvg)
library(DiagrammeR)
...
svm_node<-function(data, val=0.50){
  ...
  if(length(best)>0){
    ...
    errors_valid_nodes = 0
      ...
      ypred=predict(tune.out$best.model ,test2)
      tab1 = table(predict=ypred, truth=test2$labs_svm)
      errors_train_nodes = mean(ypred== test2$labs_svm )

      #validation
      #plot(svmfit, valid, Eccententricity ~ SP)

      ypred=predict(tune.out$best.model ,valid2)
      tab2 = table(predict=ypred, truth=valid2$labs_svm)
      errors_valid_nodes = mean(ypred== valid2$labs_svm )
    ...
    ####################
    #cost
    results[[13]] = tune.out$best.model$cost
    #degree
    results[[14]] = tune.out$best.model$degree
    #coef0
    results[[15]] = tune.out$best.model$coef0
    #########################
    ...
    tune.out<-tune(svm, as.factor(labs_svm) ~.,
              data=test2,
              kernel='polynomial',
              ranges=list(cost=c(1, 10), coef0=c(1:2, 50), degree=c(1:3, 5)),
              validation.x=valid2[,-1],
              validation.y=as.factor(valid2[,1]),
              tunecontrol = tune.control(sampling = "fix"))
...
#train the tree
TrainDMG <- function(node, data, val) {
  ...
  if (IsAlmostPure(data)) {
    ...
    #add parameters
    child1$Cost = node_new[[13]]
    child1$Degree = node_new[[14]]
    child1$coef0 = node_new[[15]]
    child0$Cost = node_new[[13]]
    child0$Degree = node_new[[14]]
    child0$coef0 = node_new[[15]]
    ...
    if(IsPure(node_new[[7]])){
      ...
      TrainDMG(child1, node_new[[7]], val )
    ...
    if(IsPure(node_new[[10]])){
      ...
      TrainDMG(child0, node_new[[10]], val)
